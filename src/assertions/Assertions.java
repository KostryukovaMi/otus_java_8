...
public class Assertions {
    ...
    public static void assertEquals(int minExpected, int maxExpected, int actual) {
        if (actual < minExpected || actual > maxExpected) {
            throw new AssertionError(String.format("Expected in %d...%d = %d", minExpected, maxExpected, actual));
        }
    }
    public static void assertEquals(int actual) {
        if (actual <= 0) {
            throw new AssertionError(String.format("Expected \"%s\" = %d", ">=0",actual));
        }
    }
